...
from skimage.transform import resize # 이미지 크기 변환
...
preprocess = lambda img: resize(img[35:195].mean(2), (80,80)).astype(np.float32).reshape(1,80,80)/255.
...
def train(shared_model, shared_optimizer, rank, args, info):
    ...
    while info['frames'][0] <= 8e7 or args.test: # openai baselines uses 40M frames...we'll use 80M
        ...
        for step in range(args.rnn_steps):
            ...
            state = torch.tensor(preprocess(state)) ; epr += reward
            ...
            if done: # maybe print info.
                ...
                state = torch.tensor(preprocess(env.reset()))
        ...
        writer.add_scalar('loss', loss.item(), num_frames)
        eploss += loss.item()
...
if __name__ == "__main__":
    ...
    if args.render:  args.processes = 1 ; args.test = True; display = Display(visible=1,size=(160, 210)); display.start()# render mode -> test mode w one process
